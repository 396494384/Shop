<template>
  <div class="category">
    ...
    <div class="content">
      ...
      <div class="tags">
        <h4>当前商品分类</h4>
        <el-alert v-if="categorys.length == 0" title="当前还没有商品分类!" type="error" :closable="false">
        </el-alert>
  ...
  methods: {
    categoryAdd() {
      if (this.category == "") {
        ...
        this.$http.get(`/api/categorys/add?name=${this.category}`).then(data => {
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        this.$http.get(`/api/categorys/del?id=${ id }`).then(data => {
          if (data.data.code == 200) {
            ...
            this.categorys = this.categorys.filter(item => { return item._id != id; });
          }
        });
      }).catch(() => {});
    }
